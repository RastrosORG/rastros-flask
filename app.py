...
def get_db_connection():
    # Define caminho padrão para desenvolvimento local
    dev_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'database.db')
    
    # Caminho para produção no Render (usando volume persistente)
    prod_path = '/opt/flask_data/database.db'
    
    # Verifica qual caminho usar
    db_path = prod_path if os.path.exists(prod_path) else dev_path
